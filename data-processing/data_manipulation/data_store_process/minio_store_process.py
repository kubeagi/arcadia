...
async def text_manipulate(req_json, opt={}):
    ...
    # get a minio client
    minio_client = await minio_store_client.get_minio_client()

    # 将文件都下载到本地
    for file_name in file_names:
        await minio_store_client.download(minio_client, {
    ...
    for item in file_names:
        ...
        if file_extension in ['csv']:
            ...
            # 处理PDF文件
            result = await pdf_handle.text_manipulate(req_json, {
                'file_name': file_name,
                'support_type': support_type,
                'conn_pool': opt['pool'],
                'task_id': opt['id']
    ...
    await minio_store_client.upload_files_to_minio_with_tags(minio_client, {
    ...
    await data_process_db_operate.update_status_by_id(update_params, {
        'pool': opt['pool']
    })

    # 更新数据集CR状态
    await dataset_cr.update_dataset_k8s_cr({
        'bucket_name': req_json['bucket_name'],
