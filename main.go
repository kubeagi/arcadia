...
func main() {
	...
	if err = (&retrievertrollers.RerankRetrieverReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "RerankRetriever")
		os.Exit(1)
	}
	if err = (&retrievertrollers.MultiQueryRetrieverReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "MultiQueryRetriever")
		os.Exit(1)
	}
	if err = (&retrievertrollers.MergerRetrieverReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "MergerRetriever")
		os.Exit(1)
	}
