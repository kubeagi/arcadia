...
func defaultHandler(event *sse.Event, last string) (newData string) {
	...
	case "result":
		...
		newData = data.Output.Choices[len(data.Output.Choices)-1].Message.Content
		fmt.Printf("%s", strings.TrimPrefix(newData, last))
		return newData
	default:
		fmt.Printf("%s", event.Data)
	}
