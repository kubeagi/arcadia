...
func NewScheduler(ctx context.Context, c client.Client, instance *v1alpha1.VersionedDataset, fileStatus []v1alpha1.FileStatus, remove bool) (*Scheduler, error) {
	...
	systemDatasource, err := config.GetSystemDatasource(ctx1)
	if err != nil {
	...
	executor, err := butcher.NewButcher[JobPayload](newExecutor(ctx1, c, oss, instance, fileStatus, remove), butcher.BufferSize(bufSize), butcher.MaxWorker(maxWorkers))
	if err != nil {
		cancel()
		return nil, err
	}
